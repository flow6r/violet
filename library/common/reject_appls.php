...
//查询申请状态
for ($applIDIndx = 0; $applIDIndx < count($appls); $applIDIndx++) {
    $currApplID = intval($appls[$applIDIndx]);
    ...
    if ($stmt->num_rows()) {
        ...
        if ($applStat === "已驳回") {
            $stmt->free_result();
            $db->close();
            echo "申请ID为" . $currApplID . "的申请已被驳回，请勿对同一申请重复执行驳回操作";
            exit;
        }
    } else {
        $stmt->free_result();
        $db->close();
        echo "申请ID为" . $currApplID . "的申请不存在，请确认检查后再执行驳回操作";
        exit;
    }
}
...
for ($applIDIndx = 0; $applIDIndx < count($appls); $applIDIndx++) {
    ...
    // 添加邮件正文
    $mail->Body = "<h1>您好，您提交的申请ID为" . $currApplID . "的申请被驳回</h1><br /><p>驳回原因：" . $rjctRsn . "</p>";
    // 为该邮件添加附件
